# 08-CONTRACT-GENERATOR.md
# 美容定型化契約 PDF 產生器
# 給 Claude Code 的完整實作指南

---

## 任務說明

在 `terrymon-grooming` 專案裡實作合約 PDF 產生功能。
飼主在 Kiosk 完成簽名後，系統自動產生符合衛福部規範的完整合約 PDF，
上傳到 Supabase Storage，並推送到飼主的 WebApp 文件收件匣。
...
└── lib/
    └── contract/
        ├── ContractPDF.tsx      ← React PDF 元件（主檔）
        ├── types.ts             ← 合約資料型別
        └── generator.ts         ← 產生 PDF 的工具函數
```
...
export interface ContractData {
  ...
  memberBirth:   string
  memberLegal:   string   // 法定代理人，未成年時填寫
  ...
  // 合約設定（由後台 store_settings 決定）
  cancelPct:       number    // 解約手續費 % (max 5)
  terminatePct:    number    // 終止手續費 % (max 10)
  refundDaysPre:   number    // 實施前退款天數 (max 15)
  refundDaysPost:  number    // 實施後退款天數 (max 30)
  court:           string    // 管轄法院地點

  // 其他
  specialNotes:  string
  signatureUrl:  string      // 飼主電子簽名圖 URL
...
### `src/lib/contract/ContractPDF.tsx`
...
// ── 字型（使用 @react-pdf/renderer 內建 CJK 支援）──────
// 若需要更好的中文字型，可在此註冊
...
export default function ContractPDF({ data }: { data: ContractData }) {
  ...
  const servicesDisplay = data.services
    .map(s => s.name)
    .join('、')
  ...
  return (
    ...
    >
      ...
      <Page size="A4" style={styles.page}>
        ...
        <Body>
          {`本契約於中華民國 ${rocY} 年 ${month} 月 ${day} 日由甲方攜回審閱（審閱期間至少三日）`}
        </Body>
        ...
        {hasAllergy && (
          <AlertBox>
            {`⚠️ 過敏警告：${data.petName} 對「${allergyText}」有過敏史，`
            + '請確認所有使用產品均不含上述過敏原，如有疑慮請於服務前告知乙方人員。'}
          </AlertBox>
        )}
      ...
      <Page size="A4" style={styles.page}>
        ...
        <Body indent>☑ 寵物沐浴梳理　☑ 寵物造型修剪　□ 護膚護毛處理　☑ 耳道清潔</Body>
        <Body indent>□ 趾甲修剪磨圓　□ 肛門腺清潔　　□ 相關商品之販賣　□ 其他</Body>
        ...
        {hasAllergy && (
          <AlertBox>
            {`⚠️ 本次服務特別注意：${data.petName} 對「${allergyText}」有過敏史，`
            + '乙方應確認所有使用產品及藥品均不含上述過敏原。'}
          </AlertBox>
        )}
        ...
        <Body indent>一、因天災、戰亂、政府法令、罷工等不可抗力或不可歸責於乙方之事由，致不能履行債務。</Body>
        <Body indent>二、甲方因死亡、疾病、健康情形不佳或遷移他處，致難以繼續接受本契約之服務。</Body>
...
/**
 * 在 Next.js API Route 裡產生合約 PDF
 * 回傳 Buffer，可直接上傳到 Supabase Storage
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 3. 取得下載 URL（私有 bucket 用 signed URL，有效 7 天）
    const { data: urlData, error: urlError } = await supabase.storage
      .from('contracts')
      .createSignedUrl(storagePath, 60 * 60 * 24 * 7)   // 7 天
...
## Phase 3 — 在 Kiosk 簽名頁呼叫

### 修改 `src/app/kiosk/signature/page.tsx` 的 `handleConfirm`
...
async function handleConfirm() {
  ...
  try {
    ...
    // Step 2：上傳簽名圖到 Storage
    const sigBlob = await fetch(signatureDataUrl).then(r => r.blob())
    const sigPath = `signatures/${member.id}/${Date.now()}.png`
    await supabase.storage.from('contracts').upload(sigPath, sigBlob)
    const { data: { signedUrl: sigUrl } } = await supabase.storage
      .from('contracts').createSignedUrl(sigPath, 60 * 60 * 24 * 365)
    ...
    // Step 4：呼叫 API Route 產生合約 PDF + 上傳
    const res = await fetch('/api/contract', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify(contractData),
    })
    const { success, url: contractUrl, error } = await res.json()
    if (!success) throw new Error(error)

    // Step 5：呼叫 posApi.completeService（傳入 contractUrl）
    ...
    toast.error('合約產生失敗：' + (err as Error).message)
    setSubmitting(false)
  }
}
```
...
在 `src/lib/constants.ts` 填入你的店家資料：

```typescript
export const STORE_ID      = 'your-store-uuid'      // 從 Supabase stores table 取
export const STORE_NAME    = 'TerryMon 寵物美容'
export const STORE_OWNER   = 'TerryMon 有限公司'
export const STORE_PHONE   = '04-2345-6789'
export const STORE_ADDRESS = '台中市西區精誠路 88 號'
export const STORE_TAX_ID  = '12345678'

// 合約條款設定（可移到 Supabase store_settings）
export const CONTRACT_CANCEL_PCT      = 5    // 解約手續費 % (max 5)
export const CONTRACT_TERMINATE_PCT   = 10   // 終止手續費 % (max 10)
export const CONTRACT_REFUND_PRE      = 15   // 實施前退款天數
export const CONTRACT_REFUND_POST     = 30   // 實施後退款天數
export const CONTRACT_COURT           = '台中'
```
...
```
□ npm install @react-pdf/renderer 成功
□ src/lib/contract/ 三個檔案建立完成
□ src/app/api/contract/route.ts 建立完成
□ Supabase Storage 的 contracts bucket 已建立（私有）
□ 執行 npm run build 無 TypeScript 錯誤
□ 在 /kiosk/signature 完成簽名後，PDF 正確產生
□ PDF 有三頁：資料頁、條款頁（26條）、簽署+解約書
□ 過敏史在 PDF 中顯示紅色警告框
□ 飼主 WebApp 的文件收件匣出現「美容合約」
□ 合約 PDF 可下載、可儲存
```
...
在 `src/app/api/contract/test/route.ts` 建立測試端點：
...
export async function GET() {
  const testData = {
    memberName: '林小華', memberId: 'M001', memberPhone: '0912-345-678',
    memberAddress: '台中市西區精誠路100號', memberBirth: '民國75年3月15日',
    memberLegal: '',
    storeName: 'TerryMon 寵物美容', storeOwner: 'TerryMon',
    storePhone: '04-2345-6789', storeAddress: '台中市西區精誠路88號',
    storeTaxId: '12345678', groomerName: '王美玲', signLocation: 'TerryMon 台中店',
    petName: '小怪獸', petBreed: '柯基犬', petWeight: 9.2,
    petAllergies: ['雞肉蛋白'],
    services: [
      { name: '洗澡＋剪毛（M型·中毛）', price: 1200, qty: 1, period: '當日' },
      { name: '香氛深層護毛', price: 200, qty: 1, period: '當日' },
    ],
    totalPrice: 1400, paymentMethod: '儲值折抵 + 信用卡',
    balanceUsed: 1000, cardAmount: 400, memberFee: 0,
    cancelPct: 5, terminatePct: 10, refundDaysPre: 15, refundDaysPost: 30,
    court: '台中', specialNotes: '客人要求保留耳毛。',
    signatureUrl: '', signedAt: new Date().toISOString(),
  }
...
訪問 `http://localhost:3000/api/contract/test` 直接在瀏覽器預覽 PDF。
