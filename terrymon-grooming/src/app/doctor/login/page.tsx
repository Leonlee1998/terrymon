...
export default function DoctorLogin() {
  ...
  const { register, handleSubmit } = useForm({
    defaultValues: { email: 'doctor@terrymon.com', password: 'doctor1234' },
  })
    ...
    <div className="flex-1 bg-primary-bg flex items-center justify-center p-6">
      <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-xl">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black text-ink">醫師登入</h1>
          <p className="text-slate-t text-sm mt-1">{CLINIC_INFO.name}</p>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <Input {...register('email')} type="email" placeholder="醫師電子信箱" className="h-12" />
        ...
        <p className="text-center text-xs text-slate-t mt-4">
          doctor@terrymon.com / doctor1234
        </p>
      </div>
    </div>
  )
