  ...
  if (!inProgress) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-white text-slate-t p-8">
        ...
        <p className="font-medium">目前沒有美容中的毛孩</p>
        <p className="text-sm mt-1">點擊「叫下一位」開始美容</p>
      </div>
    )
  }

  const lastGrooming = MOCK_PET_GROOMING_RECORDS.filter(r => r.petId === inProgress.petId)[0]

  return (
    <div className="flex-1 flex flex-col bg-white">
      <div className="px-5 py-3 border-b border-border-t flex-shrink-0">
        <h2 className="font-bold text-ink">美容中</h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        {inProgress.allergies.length > 0 && (
          <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="font-bold text-red-700">⚠️ 過敏史（產品請注意）</p>
            </div>
        ...
        {lastGrooming && (
          <div className="bg-surface rounded-2xl p-4 border border-border-t">
            <p className="text-xs font-semibold text-slate-t mb-2">上次美容紀錄</p>
            <p className="text-xs text-slate-t">{formatDate(lastGrooming.date)}</p>
            <p className="font-semibold text-ink mt-1">{lastGrooming.services.join('、')}</p>
            {lastGrooming.notes && (
              <p className="text-xs text-slate-t mt-1">備注：{lastGrooming.notes}</p>
            )}
          </div>
        )}
      </div>
