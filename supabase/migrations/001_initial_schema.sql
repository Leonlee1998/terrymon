...
create table medical_records (
  ...
  -- [{"medicine":"??????撥謑?","dosage":"10mg","frequency":"??∴??????,"days":5}]
...
begin
  ...
  if v_bonus > 0 then
    ...
    select
      ...
      format('?????NT$%s ?????NT$%s', p_amount, v_bonus)
    from members where id = p_member_id;
  end if;
...
begin
  ...
  if p_balance_to_use > v_max_offset then
    raise exception 'Balance offset exceeds max allowed: %', v_max_offset;
  end if;
  ...
  if p_balance_to_use + p_card_amount != p_total_amount then
    raise exception 'Payment split does not match total amount';
  end if;
